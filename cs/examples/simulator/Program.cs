...
namespace simulator
{
    ...
    { 
        ...
        {
            int numActions, numContexts, pStrategy, tot_iter, mod_iter, rnd_seed, swap_preferences_iter;
            ...
            if (!int.TryParse(args[1], out numActions) ||
                ...
                !int.TryParse(args[11], out swap_preferences_iter))
            {
                Console.WriteLine("Failed to parse input arguments!");
                Console.WriteLine("Usage: simulator.exe ml_args num_actions num_contexts minP maxP noClickCost clickCost pStrategy tot_iter mod_iter rnd_seed swap_preferences_iter");
                return;
            }

            VowpalWabbitSimulator.Run(ml_args, tot_iter, mod_iter, rnd_seed, numContexts, numActions, minP, maxP, noClickCost, clickCost, pStrategy, swap_preferences_iter);
        }
    }
}
